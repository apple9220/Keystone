...
exports = module.exports = function(req, res) {
	...
	if (!locals.authUser) {
		console.log('[auth.confirm] - No auth data detected, redirecting to login.');
		console.log('------------------------------------------------------------');
		return res.redirect('/checkout/checkout/login');
	}
	...
	var dologin = function() {
		...
		console.log('[auth.confirm] - loging in user...');
		console.log('------------------------------------------------------------');
		...
		var onFail = function(err) {
			console.log('[auth.confirm] - Failed loging in.', err);
			console.log('------------------------------------------------------------');
			req.flash('error', 'Sorry, there was an issue loging you in, please try again.');
			return res.redirect('/checkout/login');
		}

		keystone.session.login(String(locals.existingUser._id), req, res, onSuccess, onFail);
	...
	var checkAuth = function() {
		...
		async.series([
			...
			if (err) {
				console.log('[auth.confirm] - Issue loging user in.', err);
				console.log('------------------------------------------------------------');
				req.flash('error', err.message || 'Sorry, there was an issue loging you in, please try again.');
				return res.redirect('/checkout/login');
			}
		});
